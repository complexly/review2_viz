...
def format_js_value(thing):
    ...
    if type(thing) == int:
        return "{}".format(thing)
    if type(thing) == list:
        return "[" + ",".join(format_js_value(x) for x in thing) + "]"
    if type(thing) == dict:
        return (
            "{"
            + ",".join(
                format_js_value(k) + ": " + format_js_value(v) for k, v in thing.items()
            )
            + "}"
        )
        ...
        raise ValueError(
            """Object you passed in must either be a string or
                         Javascript() object."""
        )
...
class D3PlusViz(object):
    ...
    def create_container(self):
        ...
        container = """<div id='{id}' style='height:600px;'></div>""".format(
            id=container_id
        )
    ...
    def dump_html(self, data, container_id=None):
        ...
        return html_template.format(
            container_id=self.container_id, scripts=scripts, code=code
        )
...
class Treemap(D3PlusViz):
    def __init__(
        self, id=["group", "id"], value="value", name=None, color=None, tooltip=[]
    ):
        super().__init__()
        self.id = id
        if name is None:
...
class Scatterplot(D3PlusViz):
    def __init__(
        self, x="x", y="y", id="id", name=None, color=None, size=10, tooltip=[]
    ):
        super().__init__()
        self.id = id
        self.x = x
...
class ProductSpace(D3PlusViz):
    def __init__(
        ...
        HS_GRAPH_DATA = open(
            os.path.join(
                path,
                "../classifications/atlas_international_product_space_hs4_codes.json",
            )
        ).read()
        SITC_GRAPH_DATA = open(
            os.path.join(
                path,
                "../classifications/atlas_international_product_space_sitc_codes.json",
            )
        ).read()
    ...
    def generate_js(self, json_data):
        ...
        """.format(
            viz_data=format_js_value(RawJavascript(json_data)),
